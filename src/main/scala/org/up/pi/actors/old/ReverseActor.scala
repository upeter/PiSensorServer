...
object ReverseActor {
  ...
  case object Init extends Result
...
class ReverseActor extends Actor {
  ...
  def uninitialized:Receive = {
    case Reverse(msg) => sender ! NotInitialized
    case Init => ReverserFactory.loadReverser.map(asyncFun => become(initialized(asyncFun)))
  }
  ...
  def initialized(reverse: AsyncReverseFunction):Receive = {
    case Reverse(msg) => {
		    val sendy = sender
		    reverse(msg).map(reversed => sendy ! ReverseResult(reversed))
    }
  }
